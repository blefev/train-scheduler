 ...
 *
 * Important notes: A trip that spans multiple days IS allowed, but it is
 * factored into the weight for calculating the total trip time. It is ignored
 * for shortest riding time.
 *
 * A trip is considered to be non-stop when the difference between arrival and
 * departure of connecting trains is at most 1 minute.
 *
 */
#include "graph.h"
#include <fstream>
...
void print_itenerary(GRAPH *graph, map<int, string> &stations,
                     vector<int> path);
void lookup_station_id(GRAPH *graph, map<int, string> &stations);
void lookup_station_name(GRAPH *graph, map<int, string> &stations);
void check_service_availability(GRAPH *graph, map<int, string> &stations,
                                bool nonstop);
  ...
  while (trainsFile >> from) {
    ...
    if (arrive < depart) {
      cout << "Route " << from << " to " << to
           << " arrives earlier than it departs!\n";
...
void print_schedules(GRAPH *graph, map<int, string> &stations) {
  ...
  for (map<int, string>::iterator it = stations.begin(); it != stations.end();
       ++it) {
...
void print_schedule(GRAPH *graph, map<int, string> &stations, int station_id) {
  ...
  if (schedule.empty()) {
    ...
    // print schedules
    for (vector<vector<int>>::iterator it = schedule.begin();
         it != schedule.end(); ++it) {
...
void print_itenerary(GRAPH *graph, map<int, string> &stations,
  ...
  for (int i = 0; i < path.size() - 1; i++) {
    ...
    if (!prev_sched.empty()) {
      ...
      cout << "  " << hours << "h" << minutes << "m layover at "
           << stations.at(A) << "\n\n";
    }
...
void find_route(GRAPH *graph, map<int, string> &stations, bool count_layovers) {
  cout << "\nFind Route with Shortest"
       << (count_layovers ? " Riding " : " Total ") << "Time\n";
  cout << "---------------------\n";
